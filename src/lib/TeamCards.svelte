...
<section id="Team">
  <h2
    class="section-title"
    style="text-decoration-color: var(--baseText); -webkit-text-decoration-color: var(--baseText)"
  >
    {$t("content.home.team.main_title")}
  </h2>
    ...
    {#each staff as { name, position, descr, email, twitter, linkedin, img, bg }, i}
      <figure class="card">
        ...
        <figcaption class="slanted-bg" class:bgselected={selected === i}>
          ...
          <div
            style="max-width:100%; display:flex; justify-content:space-around; max-height:60px; "
          >
            <div class="socials" style="width:45%">
              <div class="socials-item">
                <a href={twitter} class="twitter"
                  ><img alt="Twitter" src="images/twitter-aqua.png" /></a
                >
              </div>
              <div class="socials-item">
                <a href="mailto:{email}"
                  ><img alt={email} src="images/email-nocircle-aqua.png" /><a />
                </a>
              </div>
              <div class="socials-item">
                <a href={linkedin}
                  ><img alt="LinkedIn" src="images/linkedin-aqua.png" /></a
                >
              </div>
            </div>
            {#if selected === i}
              <button
                ...
                data-card-id={i}>Less Info</button
              >
            {:else}
              <button
                class="more-info"
                style="border-color: {bg}"
                on:click={toggleMoreInfo}
                data-card-id={i}>More Info</button
              >
            {/if}
          </div>
        </figcaption>
